...
These are the definition, the theory. In this chapter I will try to explain these concepts in a more grounded, practical way. Instead of theory I'm going to start with simple examples and build from there. I'm hoping that this will help you gain a better understanding.
...
2. `next start`: the prepared packages start running on the server (a local server in our case). The server is now ready to receive requests: f.e. a user visiting a url or an app making an api call. When the server does something after receiving a request, then the server runs at `request time`. Anything that happens on the server f.e. rendering or data fetching is `server-side`.

In the build step, `Next` looks at each route and decides if it should be rendered static or dynamic.

## Rendering

But what is the rendering part in the term static and dynamic rendering?

In our example, we have `React` components in `TypeScript`. Browsers can't work with this. They need HTML, JS and CSS. Rendering means turning `React` components into HTML pages.

The difference between static and dynamic rendering is _when_ the rendering happens. Static rendering happens at `build time` (when running `next build`), dynamic rendering happens at `request time` (after `next run`, when the server receives a request).
...
Let's go over this again. At `build time`, `Next` takes a look at the route `/dynamic`. It checks all the components that make up the route. In the page component (`/app/dynamic-example-route/page.tsx`), `Next` encounters the `searchParams` page prop and decides this route cannot be statically rendered because there are no `searchParams`. Therefor this route becomes dynamically rendered.

The `searchParams` prop is not the only thing that forces a route to become dynamic. Here is the full list of api's that trigger dynamic rendering:
...
We ran `next build`. The route `/dynamic-example-route` has not been rendered. This will only happen at `request time`, that is when we start up the server and it receives a request. But, the other route - `/static-example-route` - has been rendered (prerendered). Where is it then? In the packages `Next` just build and placed in the `.next` folder:

[insert nextfolder]

I don't know what 99% of the things in this folder are. But, that doesn't matter. It's under the hood `React` and `Next` stuff. What we are interested in is this subfolder: `./next/server/app`. This holds all the routes for the `Next app router`. (There is also a folder `pages` for the old `pages router`).
...
The `html` file gets served on **`initial load`**. The `initial load` is when a user enters your app, f.e. when redirected by google or by directly entering an url in the address bar and hitting enter. On `initial load`, `Next` serves the prerendered `html`.
...
All the components in our examples thusfar have been **server components**. All the prerendering we looked into is how server components are prerendered. **Client components** work mostly differently. We will explore this in the next chapter.
...
What does dynamic rendering do? Exactly the same as static rendering. That is, create a lot of files, including an `html` file and an `rsc` file. It just does it at `request time`, using a _fresh_ request.

I can't show you these files, they are rendered `server-side` and are not saved.
...
`{ cache: 'no-store' }` means no cache. We opt out. We don't want cached data, we want to go out to this url **now** and retreive the latest data. Not some old, stale data.
...
Static and dynamic rendering should now be pretty concrete. It should also be clear how the process of static rendering leads to something we call a cache: `Full Route Cache`. Since there are no dynamic elements, every user gets served the same content. We prerender this content in advance during `build time`.
