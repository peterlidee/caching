...
We made 2 fetches from a dynamic route (auto and force-cache). Neither of them are in the `data cache`. This is unexpected because we didn't think this thru. A dynamic route doesn't run at `build time`. It runs at `request time`, when we already started up the server (`next start`). Since we haven't done this, these routes haven't been rendered yet. No fetches have been made and nothing could be cached.
...
Spoiler, I don't know. There is probably a clever reason for this and it's probably not a bug. But I don't know why that file was cached. `Gemini` confidently suggested that it has something to do with revalidation...

But it doesn't matter. We will just assume that this cache has some purpose outside of what we learned about the `data cache`. It doesn't matter for us because it doesn't change what we learned. And we'll leave it at that.
