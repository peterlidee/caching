...
It's best explained using an example. Note: the examples are available on [github](https://github.com/peterlidee/caching).
...
Quick overview: We're trying to simulate that we have a CMS that returns 5 posts. So, we take the 100 posts that [JSONPlaceholder api](https://jsonplaceholder.typicode.com/posts) returns and only use the first 5. We then render these 5 posts out as links to individual blog posts. Finally, I manually added a 6th link. We will need this in a bit to run some tests.
...
On our blog home page, we made a fetch with no caching option - the `auto` option. That means no `data cache` was created. `Next`made a fetch, used it to generate `HTML` and `rsc` but did not cache the fetch result. In a real project, that could be a problem: when we publish a new blogpost, we would like it to appear on our home page.How would we solve this?

1. We could make the blog homepage dynamic and set no-store to the fetch. Every request would generate a fresh result. It would work.
2. `revalidatePath` or `revalidateTag` could work. CMS (publish post) -> webhook -> route handler -> `revalidatePath('/app/blog')` or `revalidateTag(posts)`. The cached route is marked as stale and `Next` regenerates the page. We now have a link to our new blog post.
3. Time based revalidation could also work, e.g. `fetch('https://jsonplaceholder.typicode.com/posts', { revalidate: 360 })`. Every hour the data becomes stale and the next request will trigger a revalidation. Result: our new link appears.
