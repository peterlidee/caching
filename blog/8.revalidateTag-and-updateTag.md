...
- Don't use it in `server actions` (you can, but use `updateTag` instead).
...
Note: the examples are available on [github](https://github.com/peterlidee/caching). We make a new route handler that will call `revalidateTag`. As before, we will hit this route handler directly to simulate an external call:
...
- The `Next` docs explicitly mention that it's fine to call both `revalidatePath` and `updateTag` together to ensure data consistency.
- Caching and revalidation work completely differently in development mode, so be careful.
