...
We are testing calling `revalidatePath` a route handler. This route handler would get called from an external place, f.e. a CMS webhook. To simulate this, we will use a button in a new route `/revalidate/path/external`. This button will call the route handler.

This is our route handler, where we call `revalidatePath`:
...
So, this button calls our route handler. We make a fetch to our api endpoint and save the result in state. The component returns the button and an error or success state. We then load the button in it's own route:

```tsx
// app/revalidate/path/external/page.tsx
...
So route -> button -> route handler -> revalidatePath. It looks like this when clicked:
...
[action]
  next build
  next start
  visit /revalidate/path/external (button page)
  click button (we assume success)
  visit /revalidate/path/swr (posts page that we just revalidated)

[Expect]
  ✅ pass: Stale content, all posts to have the same timestamp (from build)
...
One more test. We have our route `/revalidate/path/swr` with 3 posts: 35-37. We make a copoy of this route with the same 3 posts: `/revalidate/path/swr-copy`. The goal is to test if this route copy gets revalidated when we revalidate the original route.
