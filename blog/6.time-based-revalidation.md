...
In time based revalidation, an expiry date is added to cache entries. However, when a cache entry expires (becomes stale) it does not update automatically. There are no server agents or CRON jobs that look for expired cache and update them.
...
Note: the examples are available on [github](https://github.com/peterlidee/caching). Let's first take a look at a generated `data cache` file. We're going to reuse our `<Post />` component we used in an earlier chapter:
