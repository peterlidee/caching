...
And we load this inside this route:

```tsx
// app/revalidate/time/simple/page.tsx
...
├ ○ /revalidate/time/simple         30m      1y

○  (Static)   prerendered as static content
```

The `Revalidate` value 30m is what we set as revalidate in our fetch. This is the expiry date of the route in the `full route cache`. We talked about this earlier. This is equal to the lowest revalidate value in the entire route.
...
After running build, we expect:

1. Route to be statically rendered and have expiry of 60s -> ✅ passes
2. Fetches 30 to be in `data cache` with revalidate 60 -> ✅ passes
3. Fetches 31 to be in `data cache` with revalidate 600s -> ✅ passes
4. Fetches 32 to be in `data cache` with revalidate 1 year -> ✅ passes
5. Fetches 30-32 to have +/- same create date (headers.date) -> ✅ passes

After running `next start` and visiting this route more than 1 minute but less then 10 minutes after we created it, we expect SWR behaviour for fetch 30:

1. Swr for fetch 30: user was served stale data (we can't check this because we refetched the same data, so no point)
2. Fetch 30 to have been updated in the background: new create date -> ✅ passes
3. Fetch 31 and 32 not to have changed (unchanged create date) -> ✅ passes

10 minutes later, after performing hard page reload, we expect:

1. Fetch 30 to have been updated in the background again: new create date -> ✅ passes
2. Fetch 31 to have been updated in the background: new create date -> ✅ passes
3. Fetch 32 not to have changed (unchanged create date) -> ✅ passes
