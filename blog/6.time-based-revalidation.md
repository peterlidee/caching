...
We note the create date for the fetches in the data cache.

1. 14:02:33

We run `next start` and visit route `/revalidate/time/swr` more than 1 minute but less then 10 minutes after we created it.

We expect SWR behaviour for fetch 30. We expect:

1. Stale behaviour in browser: old time still displayed for all fetches. -> ✅ passes
2. Revalidated in cache: new create date for fetch 30: -> ✅ passes
3. Fetch 31 and 32 not to have changed (unchanged create date) -> ✅ passes

We perform hard refresh of page. We expect:

1. Time in browser for fetch 30 to have updated -> ✅ passes

10 minutes later, after performing hard page reload, we expect:

1. Time in browser for fetch 30 and 31 to stay the same -> ✅ passes
2. Fetch 30 to have been updated in the background again: new create date: -> ✅ passes
3. Fetch 31 to have been updated in the background: new create date -> ✅ passes
4. Fetch 32 not to have changed (unchanged create date) -> ✅ passes

One last refresh:

1. Time in browser for fetch 30 and 31 updated -> ✅ passes
