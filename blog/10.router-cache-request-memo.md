...
`Router cache` is client side cache, it lives in the browser's memory and consists of `rsc` payload. The goal of `router cache` is instantanious navigation between pages. It does that by caching already visited pages and by prefetching pages that we might visit (the `<Link />` component) and saving those in cache.
...
I can't improve what the `Next` docs say about this, so I'm just gonna copy it:
...
You may be thinking I took it a bit easy this chapter by just copy-pasting. True, it was easier but there are some reasons why I did it.

1. `Next` just explained it reasonably well. There was nothing for me to clarify.
2. It's hard to write tests for this so I couldn't really offer any added value.
...
It's very common to need data from a particular fetch in multiple locations (components) in a route. To optimize your app, you would then have to pass this data around via props or context for example. `Request memoization` saves you from having to do this by caching the result of the fetch and reusing it.
...
In the above example, only 2 fetches will be made, fetch 1 and fetch 2. When `Next` hits the same fetch again, it will reuse the memoized result.
...
2. `Request memoization` has a lifespan of the request cycle. So, a request is made, fetches are memoized and reused if needed, the route renderes and the `request memoization` cache is removed.

So, the big point of memoization is that you can be 100% confident in making the same fetch over and over again in a singe route. It will not affect performance or cause higher load on some endpoint.
...
A get user function that calls this endpoint. Note, we pass fetch options as param:
...
So, `getUser` was called 20 times and after that the api was called once. I am not sure I can fully explain this. One factor is that `request memoization` is a `React` thing, so it won't recognize `options.next` settings. (It will recognize `options.cache`). That would leave 3 calls with the same url but different options:

- `<User fetchOptions={{}} />`
- `<User fetchOptions={{ cache: 'no-store' }} />`
- `<User fetchOptions={{ cache: 'force-cache' }} />`

But there was only one call to the api. Of course, each of these would return the same result and it seems that `React` is smart enough to understand this. So concluding, eventhough we had fetches with the same url but different options, `request memoization` stepped in and only made the fetch once. Great!

**Bonus points**: Here's a little question: how many of these fetches ended up in `data cache`?

Again, a bit surprising, we have 2 entries in cache:
...
`React` automatically does `request memoization` for the `fetch API`. If you don't use fetch, you can manually use `request memoization` by wrapping a function with `cache`:
...
To manually add items to `data cache`, `Next` provides the [`unstable_cache` function](https://nextjs.org/docs/app/api-reference/functions/unstable_cache). I won't expand on this, you can read the docs. Do note that `Next` _warns_ that this API was replaced with `use cache`. We cover this later.
